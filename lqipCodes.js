...
/* terminal command to use: node lqipCodes.js */

import { getPalette } from "colorthief";
import sharp from "sharp";
import rgbToOkLab from "color-core";
import fs from 'node:fs';

const rgbToOklabFunction = rgbToOkLab.rgbToOklab;

async function analyzeImage(imagePath) {
    imagePath = Buffer.from(imagePath);
    ...
    const cells = Array.from({ length: 6 }, (_, index) => {
        ...
        return rgbToOklabFunction({ r, g, b });
    });
...
const img = await new Promise((resolve, reject) => {
    fs.readFile("./src/image.jpg", (err, data) => {
        if (err) {
            reject(err);
        } else {
            resolve(data.buffer); // Convert Buffer to ArrayBuffer
        }
    });
});
